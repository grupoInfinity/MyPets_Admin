...
if(strtoupper($accion) =='C'){ //VERIFICACION SI LA ACCION ES CONSULTA
	...
	if(strtoupper($accion) =='I'){// VERIFICACION SI LA ACCION ES INSERCION
		...
		$sql = "SELECT DISTINCT mn.*  
		FROM sec_menu mn
		INNER JOIN sec_opc_rol opcr ON opcr.id_menu = mn.id_menu
		INNER JOIN sec_rol_usuario rs ON rs.usuario = '$user' AND rs.id_rol = opcr.id_rol
		WHERE mn.estado = 'A'";
		...
		FROM $db.sec_rol_usuario rs
		INNER JOIN $db.sec_opc_rol opcr ON opcr.id_rol = rs.id_rol
		INNER JOIN $db.sec_opcion opc ON opc.id_opc = opcr.id_opc 
		AND COALESCE(opc.id_menu, -1) = COALESCE(opcr.id_menu, -1)
		AND opc.estado = 'A'
		INNER JOIN $db.sec_usuario us ON us.usuario = rs.usuario AND us.estado = 'A' AND us.usuario = '$user'";
