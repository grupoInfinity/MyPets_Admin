		...
		if ($result->num_rows > 0) {
			...
			while ($row = $result->fetch_assoc()) {
				...
				$results[] = array(
					...
					"usr" => $usr, "id_empleado" => $id_empleado, "id_almacen" => $id_almacen, 'clave' => utf8_encode($clave), 'nombre' => utf8_encode($nombre), 'apellido' => utf8_encode($apellido), 'email' => utf8_encode($email), 'cargo' => utf8_encode($cargo), 'id_cargo' => ($id_cargo), 'puesto' => utf8_encode($puesto), 'id_puesto' => ($id_puesto), 'tipo_usuario' => utf8_encode($tipouser), 'estado' => $estado, 'ctg_empleado' => $ctg_empleado
				);
