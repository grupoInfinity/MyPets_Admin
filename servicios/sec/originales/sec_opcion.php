...
if(strtoupper($accion) =='C'){ //VERIFICACION SI LA ACCION ES CONSULTA
    ...
    $sql = "SELECT A.id, A.id_opc_principal, A.id_opc_padre, A.padre, A.descripcion, A.url, A.estado, A.id_empresa, A.orden
	FROM $bd.$tabla A
    WHERE $id_opc $id_opc_ppal $id_opc_padre $desc $estado $id_empresa $padre
    ...
    if (!empty($result))
        if($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                $id_opc = $row["id"];
                $id_opc_ppal  = $row["id_opc_principal"];
