...
if (strtoupper($accion) == 'C') { //VERIFICACION SI LA ACCION ES CONSULTA
	...
	if (!empty($id_menu)) $id_oid_menu = "AND A.id_menu=$id_menu";
	else $id_menu = "";
	...
	if (strtoupper($accion) == 'I') { // VERIFICACION SI LA ACCION ES INSERCION
		...
		if (!empty($result)) {
			if ($result->num_rows > 0) {
				while ($row = $result->fetch_assoc()) {
					$results[] = array(
						"id_menu" => $row["id_menu"], 'descripcion' => utf8_decode($row["descripcion"]), 'menu_icon' => utf8_decode($row["menu_icon"]), 'orden' => ($row["orden"])
					);
					$json = array("status" => 1, "info" => $results);
				}
			} else {
		...
		if (!empty($result))
			if ($result->num_rows > 0) {
				while ($row = $result->fetch_assoc()) {
					$results[] = array(
						"id" => $row["id_opc"], 'id_menu' => ($row["id_menu"]), 'id_opc_padre' => ($row["id_opc_padre"]), 'padre' => ($row["padre"]), 'descripcion' => ($row["descripcion"]), 'url' => ($row["url"])
					);
					$json = array("status" => 1, "info" => $results);
				}
			} else {
